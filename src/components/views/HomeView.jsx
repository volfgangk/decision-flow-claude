...
// =========================================================================
// 2. [TEAM CARD] — 팀 카드 (실제 안건 수 표시)
// =========================================================================
const TeamCard = ({ team, index, onClickTeam, firebaseAgendas }) => {
  const teamAgendas = firebaseAgendas.filter(a => a.team_id === team.id);
  const activeCount = teamAgendas.filter(a => a.status !== '마감').length;
  const doneCount = teamAgendas.filter(a => a.status === '마감').length;
...
const HomeView = ({
  ...
  firebaseAgendas, onHomeCreateAgenda
}) => {
    ...
    <>
      ...
      <main className="flex-1 px-5 pb-24 overflow-y-auto bg-white flex flex-col">
        ...
        <div className="flex gap-0 mb-3 shrink-0">
          <button onClick={() => setActiveTab('created')}
            className={`flex-1 py-2.5 text-[12px] font-black rounded-l-xl border-2 transition-all ${
              activeTab === 'created' ? 'bg-[#FFF0F3] border-[#FCD9E1] text-[#C95374]' : 'bg-gray-50 border-gray-200 text-gray-400'
            }`}>
            내가 만든 팀
          </button>
          <button onClick={() => setActiveTab('joined')}
            className={`flex-1 py-2.5 text-[12px] font-black rounded-r-xl border-2 border-l-0 transition-all ${
              activeTab === 'joined' ? 'bg-[#EEF3FF] border-[#C5D5F7] text-[#4A6FA5]' : 'bg-gray-50 border-gray-200 text-gray-400'
            }`}>
            내가 참여한 팀
          </button>
        </div>
        ...
        <div className={`rounded-2xl p-3.5 mb-4 border-2 ${
          activeTab === 'created' ? 'bg-[#FFF0F3] border-[#FCD9E1]' : 'bg-[#EEF3FF] border-[#C5D5F7]'
        }`}>
          {teamsLoading ? (
            ...
            <div className="space-y-2">
              {currentTeams.map((t, i) => (
                <TeamCard key={t.id} team={t} index={i} onClickTeam={onClickTeam} firebaseAgendas={firebaseAgendas} />
              ))}
              <button onClick={activeTab === 'created' ? onOpenCreateTeam : onOpenJoinTeam}
                ...
                {activeTab === 'created' ? <><Plus className="w-3.5 h-3.5" />새 팀 만들기</> : <><KeyRound className="w-3.5 h-3.5" />초대코드 입력</>}
              </button>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center py-6 px-4 text-center">
              ...
              <p className="text-[11px] font-black text-gray-400 mb-1 leading-relaxed">
                {activeTab === 'created' ? '팀을 만들면 안건을 공유할 수 있어요' : '초대코드를 받으셨나요?'}
              </p>
              <button onClick={activeTab === 'created' ? onOpenCreateTeam : onOpenJoinTeam}
                className={`mt-3 rounded-xl py-2 px-4 flex items-center gap-1.5 shadow-md text-[11px] font-black active:scale-[0.98] text-white ${
                  activeTab === 'created' ? 'bg-gradient-to-r from-[#E8668A] to-[#F4A067]' : 'bg-gradient-to-r from-[#5B8DEF] to-[#7C6CF0]'
                }`}>
                {activeTab === 'created' ? <><Plus className="w-3.5 h-3.5" />첫 팀 만들기</> : <><KeyRound className="w-3.5 h-3.5" />초대코드 입력</>}
              </button>
            </div>
          )}
        </div>
              ...
              {recentActive.map(item => (
                <div key={item.id} onClick={() => onSelectId(item.id)}
                  ...
                  <div className="flex items-center gap-1.5 text-gray-400">
                    ...
                    <span className="text-[10px] font-medium">{item.voters || 0}명이 참여 중</span>
                  </div>
                </div>
              ))}
            </div>
